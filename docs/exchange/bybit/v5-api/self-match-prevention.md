...
> Source: https://bybit-exchange.github.io/docs/v5/smp
...
Check request params of [Place Order](https://bybit-exchange.github.io/docs/v5/order/create-order). Specify parameter `smpType` when placing the order
...
1. The relationship between SMP Trade Group and UIDs can be changed: when a UID joins a new SMP Trade Group or is
...
- Bybit compliant sites: SMP is enabled by default for all users.Note: Most compliant sites currently support spot trading only.
